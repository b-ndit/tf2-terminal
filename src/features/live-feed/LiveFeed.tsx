...
function describeEvent(event: ListingEvent): { symbol: string; label: string; colorClass: string } {
  if (event.kind === "removed") {
    return { symbol: "▼", label: "delist", colorClass: "text-zinc-500" };
  }
  if (event.intent === "buy") {
...
export function LiveFeed() {
  ...
  return (
    <div className="flex h-full min-h-0 flex-col overflow-y-auto bg-charcoal p-4 text-zinc-200">
      ...
      {isLoading && !error && <p className="text-sm text-zinc-500">Loading recent listings…</p>}
      {!isLoading && !error && feed.length === 0 && (
        <p className="text-sm text-zinc-500">No listing activity observed yet.</p>
      )}
  ...
  return (
    <div className="flex items-center justify-between gap-3 border-b border-charcoal-border px-3 py-1.5 text-sm last:border-0">
      ...
      <div className="flex shrink-0 items-center gap-2 text-xs text-zinc-400">
        ...
        {event.value_ref !== null && <span className="font-medium text-zinc-200">{event.value_ref.toFixed(2)} ref</span>}
      </div>
    </div>
  );
}
