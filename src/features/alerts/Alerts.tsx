    ...
    <div className="flex h-full min-h-0 flex-col overflow-y-auto bg-charcoal p-4 text-fg">
      ...
      >
        <div className="flex min-w-[240px] flex-1 flex-col gap-1">
          <label className="text-xs text-fg-muted">Item (classifieds URL)</label>
          <input
            type="text"
            value={url}
            onChange={(e) => setUrl(e.target.value)}
            placeholder="Paste a backpack.tf classifieds URL…"
            className="rounded border border-charcoal-border bg-charcoal px-3 py-2 text-sm placeholder:text-fg-subtle focus:outline-none"
          />
        </div>
        ...
        {needsThreshold && (
          <div className="flex flex-col gap-1">
            <label className="text-xs text-fg-muted">Threshold ({kind === "spread_widen" ? "%" : "ref"})</label>
            <input
              type="number"
              step="0.01"
              value={threshold}
      ...
      <div className="mb-6">
        <h3 className="mb-2 text-sm font-medium text-fg-muted">Rules ({rules.data?.length ?? 0})</h3>
        <div className="rounded border border-charcoal-border">
          {(rules.data ?? []).length === 0 ? (
            <p className="px-3 py-3 text-sm text-fg-subtle">No alert rules yet.</p>
          ) : (
            rules.data!.map((rule) => (
              <div
                key={rule.id}
              ...
              >
                <div className="min-w-0">
                  ...
                  <span className="ml-2 text-xs text-fg-subtle">[{rule.channels.join(", ")}]</span>
                </div>
                <div className="flex shrink-0 items-center gap-2">
                  <button
                    ...
                    className={`rounded px-2 py-1 text-xs ${
                      rule.enabled ? "bg-charcoal-raised hover:bg-charcoal-border" : "bg-charcoal-raised text-fg-subtle hover:bg-charcoal-border"
                    }`}
                  >
      ...
      <div>
        <h3 className="mb-2 text-sm font-medium text-fg-muted">Recent Alerts</h3>
        <div className="rounded border border-charcoal-border">
          {recent.length === 0 ? (
            <p className="px-3 py-3 text-sm text-fg-subtle">No alerts fired yet.</p>
          ) : (
