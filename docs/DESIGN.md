...
- **Trade offers:** `IEconService/GetTradeOffers` (user's own API key — officially supported for acting on your own account). Polled at a respectful interval; v1 shipped analysis-only, no send/accept/decline (kept things clearly inside Steam's ToS at the time). **That's since changed by explicit user decision — see §9's addendum after the Module 15 backpack-click note** for `infra::steam::trade_send::SteamSessionClient`, which uses separate session-cookie auth (not this API key) for the unofficial send/accept/decline endpoints. **Deviation (found live, Module 15 verification):** the first real call against a live account failed with an opaque "error decoding response body" — Steam's Web API is inconsistent about whether large integer fields (`tradeofferid` here) come back as JSON numbers or JSON-quoted strings, and serde doesn't coerce between the two by default. `TradeOffer::tradeofferid` now accepts either representation. `SteamApiClient::get_json` also now fetches the raw body and logs a snippet on any future decode failure — the original `reqwest`-only error gave no way to see what Steam actually sent, which is what made this one hard to diagnose from the error message alone.
...
**Deviation (Module 15, found live):** Flip Finder's numbers were being skewed by Halloween-Spelled listings — e.g. a real captured buy order was "BUYING SPELLED (57 keys), LESS FOR NON-SPELLED" on a Strange Killstreak Australium Eyelander otherwise worth a few keys. Spells aren't a dimension of `domain::item::ItemKey` (Valve's own schema has no defindex/quality distinction for them — they're a description-only attribute on the exact asset), so a spelled listing's price had no home in per-SKU aggregation and was silently mixed into the plain item's buy/sell pool. The real websocket payload turns out to carry spells as a genuine structured field (`item.spells: [{name: "Exorcism", ...}]`, verified live), not something that needs fragile free-text matching against the listing's `details` message — `WsItem::has_spells` checks it, and `MarketDataService::spawn_listener`'s ingestion loop drops any spelled listing before it ever reaches `market_listings`/the broadcast bus, so every downstream consumer (History Recorder, Flip Finder, Market Analyzer, Alerts) is protected for free. Other "buying for a specific reason" signals besides spells (e.g. paint-specific requests) only exist as free text in `details` and aren't filtered — that's a fragile keyword-matching problem, intentionally not attempted without a concrete case to verify against.
...
Scoped deliberately short of the fuller "list on backpack.tf" / "quicksell" request that prompted this: backpack.tf's v2 API does support creating a classifieds listing given the user's token, but **actually sending a Steam trade offer does not work with just a Web API key** — Steam's official `IEconService` only exposes read/manage-your-own-offers endpoints; creating a new offer requires `steamcommunity.com/tradeoffer/new/send`, authenticated with real browser session cookies (`sessionid` + `steamLoginSecure`), not the API key model this app deliberately uses instead (§2: "we never see credentials"). That's also explicitly unofficial/unsupported by Valve.

**That line has since been crossed, as an explicit user decision** (post-Module-15): `infra::steam::trade_send::SteamSessionClient` calls `tradeoffer/new/send`/`/accept`/`/decline` using two session cookies the user pastes in manually from their own browser (Settings → "Steam Session" — mirrors the existing `SecretField` pattern for the Steam API key/backpack.tf token, not an embedded-webview capture flow, so no new Tauri window/capability was needed). This is the one place in the app that holds something closer to a live credential than an identifier or API key; the Settings UI says so explicitly (unofficial endpoint, real account risk, not covered by Steam's own Web API terms). `get_active_trades`/`TradeAnalysisEngine` are unaffected — they still only read via `IEconService`. The Trade Analyzer's Accept/Decline buttons require an explicit confirm step (irreversible against a real account) and degrade to a "connect a session" hint if it isn't configured. A minimal "propose a trade" flow (Backpack multi-select → partner SteamID64 + item pickers → `send_trade_offer`) covers sending; it's deliberately not the fuller drag-drop builder Module 13 already scoped out.
